 ...
 */
		...
		function SetLicensePath()
		{
			...
			if(is_file("conf/scriptcase.nm"))
			{
				$str_license_path = getcwd() . "/conf - scriptcase.nm";

				$hd = fopen("conf/scriptcase.nm", "r");
				$arq = fread ($hd, filesize ("conf/scriptcase.nm"));
				fclose($hd);
				$arq = chunk_split($arq, 100, '<br>');
				$str_license_path .= "<BR>Dados:<BR> " . $arq;
			}elseif(is_dir($str_license_path))
			{
				$str_license_path = get_cfg_var('zend_optimizer.license_path');
				$old_dir = getcwd();
				chdir($str_license_path);
				if(is_file("scriptcase.zl"))
				{
						$str_license_path .= "  - scriptcase.zl";

						$hd = fopen("scriptcase.zl", "r");
						$arq = fread ($hd, filesize ("conf/scriptcase.zl"));
						fclose($hd);
						$arq = chunk_split($arq, 100, '<br>');
						$str_license_path .= "<BR>Dados:<BR> " . $arq;
				}
				...
				$str_license_path = "SEM ARQUIVO";
			}

			$this->str_license_path = $str_license_path;
		}//Seta diretorio da licenca

		//Pega diretorio da licenca
		function GetLicensePath()
		{
				return $this->str_license_path;
		}//Pega diretorio da licenca
...
class diagnosis
{
        ...
        function makeDiagnosis()
        {
                ...
                //checa license path
                $teste_tmp = $obj_ambiente->GetLicensePath();
                $imagem = "button_ok.png";
                if(empty($teste_tmp))
                {
                        $imagem = "button_cancel.png";
                ...
                $this->escreveTela("    </TD>\r");
                $this->escreveTela("    <TD align='left' ". $cssText .">\r");
                $this->escreveTela("      ".$arr_lang['zend_licence_path'].": " . $teste_tmp . "\r");
                $this->escreveTela("    </TD>\r");
                $this->escreveTela("  </TR>\r");
                //checa license path
