...
if (!isset($this->sc_temp_usr_login)) { $this->sc_temp_usr_login = (isset($_SESSION['usr_login'])) ? $_SESSION['usr_login'] : "";}
 ...
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 { 
$this->nmgp_redireciona_form($this->Ini->path_link . "mensajes_cambio_contrasena/mensajes_cambio_contrasena.php", $this->nm_location, "mensaje?#?" . 'EL CAMBIO DE CONTRASEÑA SE REALIZO SATISFACTORIAMENTE' . "?@?", "_self", "ret_self", 440, 630);
 };
if (isset($this->sc_temp_usr_login)) {  $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
$_SESSION['scriptcase']['cambiar_contrasena']['contr_erro'] = 'off'; 
          }
    ...
    function ValidateField_old_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          if ($this->old_pswd == "")  
          { 
              $Campos_Falta[] =  "Contraseña Actual" ; 
      ...
      { 
          ...
          { 
              $Campos_Crit .= "Contraseña Actual " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 32 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['old_pswd']))
              {
                  $Campos_Erros['old_pswd'] = array();
              }
    ...
    function ValidateField_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $Campos_Crit .= "Nueva Contraseña " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 32 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['pswd']))
              {
                  $Campos_Erros['pswd'] = array();
              }
    ...
    function ValidateField_confirm_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $Campos_Falta[] =  "Repita Contraseña" ; 
      ...
      { 
          ...
          { 
              $Campos_Crit .= "Repita Contraseña " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 32 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['confirm_pswd']))
              {
                  $Campos_Erros['confirm_pswd'] = array();
              }
