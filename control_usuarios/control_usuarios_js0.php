...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_usuarios']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
</form> 
...
function NM_critica (form, campo, label, mask, tipo, intr, dec, min, max)
{
 ...
 NM_tab_crit[1] = new NM_critica('F1', 'login', 'Usuario:', 'naomask', 'lista', 'cxab', 30, 'TUDO');
 NM_tab_crit[2] = new NM_critica('F1', 'pswd', 'Contraseña:', 'naomask', 'lista', 'cxab', 20, 'TUDO');
 NM_tab_crit[3] = new NM_critica('NM_final');
 NM_tab_crit_1 = new NM_critica(3);
 NM_tab_crit_1[1] = new NM_critica('F1', 'login', 'Usuario:', 'naomask', 'lista', 'cxab', 30, 'TUDO');
 NM_tab_crit_1[2] = new NM_critica('F1', 'pswd', 'Contraseña:', 'naomask', 'lista', 'cxab', 20, 'TUDO');
...
{ 
   ...
   document.F4.action               = "control_usuarios_fim.php";
   document.F4.submit(); 
} 
