...
$old_error_handler = set_error_handler("reconexionWS_scriptcase_error_handler");

function reconexionWS_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['reconexionWS']['contr_erro']) && $_SESSION['scriptcase']['reconexionWS']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_reconexionWS->contr_reconexionWS->Apl_com_erro = true;
            if ($inicial_reconexionWS->contr_reconexionWS->NM_ajax_flag)
            {
                $inicial_reconexionWS->contr_reconexionWS->NM_ajax_info['errList']['geral_reconexionWS'][] = $msg;
                return;
            }
            reconexionWS_scriptcase_error_display($msg, $err_no);
        }
...
function reconexionWS_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['reconexionWS'])  && '' != $_SESSION['scriptcase']['error_icon']['reconexionWS'])  ? $_SESSION['scriptcase']['error_icon']['reconexionWS']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['reconexionWS']) && '' != $_SESSION['scriptcase']['error_close']['reconexionWS']) ? $_SESSION['scriptcase']['error_close']['reconexionWS'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['reconexionWS']) && '' != $_SESSION['scriptcase']['error_span_title']['reconexionWS']) ? $_SESSION['scriptcase']['error_span_title']['reconexionWS'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['reconexionWS']) && '' != $_SESSION['scriptcase']['error_icon_title']['reconexionWS']) ? $_SESSION['scriptcase']['error_icon_title']['reconexionWS'] : '';
?>
